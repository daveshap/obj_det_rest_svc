...
def main():
    ...
    while True:
        ...
        try:
            ...
            #print(len(images))
            for img in images:
                if img['uuid'] not in uuid_seen:
                    image_tensor = convert_from_json(img['data'])
                    detector_output = detector(image_tensor)
                    class_ids = detector_output["detection_classes"]
                    uuid_seen.append(img['uuid'])
                    meta = {'time':img['time'], 'type':'inference.sense.optical.object.detection.recognition', 'format':'TBD', 'service':svc_name, 'uuid':str(uuid.uuid4()), 'parent_uuid':img['uuid']}
